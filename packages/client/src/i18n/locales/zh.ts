export default {
  ...
  platform: {
    ...
    geweTokenHint: 'GeWe API 请求头 X-GEWE-TOKEN；每个 profile 必须唯一',
    geweAppId: 'GeWe App ID',
    geweAppIdHint: 'GeWe 回调和发送接口使用的 appId；每个 profile 必须唯一',
    ...
    geweRelayAppHint: 'webhook-router App ID/token；使用 webhook-router 时每个 profile 必须唯一',
    ...
    geweGroupPolicyHint: "控制当前 profile 的 GeWe 群聊消息入口；需要 {'@'} 时会用机器人 wxid 精确匹配",
  ...
  gewe: {
    title: 'GeWe',
    commonConfig: '配置',
    commonHint: '当前 profile 的 GeWe 配置。',
    profileConfig: 'Profile 配置',
    profileHint: '每个 profile 独立配置并运行自己的 GeWe channel，消息会直接进入当前 profile。',
    enabledHint: '启用当前 profile 的 GeWe channel。',
    ...
    relaySseAppHint: 'SSE 监听地址会由 Webhook-router 地址、App ID 和 token 自动组成，例如 /apps/app-id/events?token=xxxx；App ID 和 token 不能与其它 profile 重复。',
    relayChannel: 'Webhook-router 通道',
    relayChannelHint: '可选；Webhook-router Callback 模式下只接收指定 channel 的事件。',
    ingressConfig: '入口配置',
    ingressHint: '选择 GeWe 原生回调或 webhook-router 中转；这里只配置当前 profile 的入口。',
    deliveryConfig: '主动投递',
    deliveryHint: '定时任务、启动通知等主动消息会发到这里。',
    accessConfig: '访问控制',
    accessHint: '控制哪些 wxid 或群聊消息可以进入当前 profile。',
    unauthorizedDmPair: '发送配对提示',
    unauthorizedDmIgnore: '忽略',
    groupPolicyPaired: '授权用户',
