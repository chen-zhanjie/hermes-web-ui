export default {
  ...
  platform: {
    ...
    geweInboundModeHint: '直连 callback 不依赖 webhook-router；relay callback/SSE 通过 webhook-router 中转',
  ...
  gewe: {
    title: 'GeWe',
    commonConfig: '通用配置',
    commonHint: '同一个 GeWe 入口接收全部私聊和群聊消息，再按发送人 wxid 分发到 profile。',
    profileConfig: 'Profile 独有配置',
    profileHint: '主页频道等主动投递设置属于具体 profile，不影响共享入口。',
    sharedEnabledHint: '启用 web-ui 侧共享 GeWe 入口；回调会按 sender wxid 路由到绑定 profile。',
    callbackUrl: 'Callback 地址',
    callbackUrlHint: '把这个地址配置到 GeWe 或 webhook-router；该入口由 web-ui 统一分发到各 profile。',
    inboundHint: 'Web UI callback 是推荐模式；relay 模式用于 webhook-router 中转。',
    bindings: '绑定关系',
